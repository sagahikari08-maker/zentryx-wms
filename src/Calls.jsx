...
const Calls = () => {
  ...
  return (
    <main className="p-4 w-full max-w-[1400px] mx-auto relative z-0 animate-fade-in">
            ...
            <tbody className="text-[12px] text-gray-700">
              {filteredCalls.length === 0 ? (
                ...
                filteredCalls.map((call) => {
                  ...
                  return (
                    <tr key={call.id} className={`border-b border-gray-100 transition-colors ${
                      isCompleted ? 'bg-gray-50 opacity-80' : 
                      isMissed ? 'bg-red-50/40 hover:bg-red-50' : 'hover:bg-blue-50/40'
                    }`}>
                      ...
                      <td className="py-4 px-6 text-center">
                        ...
                        <div className="text-[10px] font-bold text-gray-500 uppercase tracking-wider mt-0.5">⏱️ {call.time}</div>
                      </td>
                      <td className="py-4 px-6 text-center">
      ...
      {/* ========================================================================= */}
      {/* 🛡️ MODAL: CREATE / EDIT CALL FORM (ENTERPRISE GRADE)                      */}
      {/* ========================================================================= */}
      {isModalOpen && (
          ...
          <div ref={modalRef} className="bg-white rounded-sm w-full max-w-[650px] shadow-2xl flex flex-col relative animate-fade-in border-t-4 border-t-[#125ab2]" style={{ maxHeight: 'calc(100vh - 2rem)' }}>
            ...
            <div className="bg-blue-50 border-b border-blue-100 px-6 py-4 flex justify-between items-center shrink-0 z-10">
              <div>
                ...
                <h3 className="font-black text-sm uppercase tracking-wider text-gray-900">{editingId ? `Update Log: ${editingId}` : 'Register New Contact Protocol'}</h3>
              </div>
            ...
            <form onSubmit={handleSave} className="flex flex-col flex-1 overflow-hidden">
              <div className="p-6 overflow-y-auto flex-1 custom-scrollbar z-0 text-sm flex flex-col gap-5">
                ...
                {/* ROW 1: Classification & Target */}
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 border-b border-gray-100 pb-4">
                  <div>
                    <label className="block text-[9px] font-bold text-gray-500 uppercase tracking-wider mb-1.5">Log Category <span className="text-red-500">*</span></label>
                    <select value={form.category} onChange={e => setForm({...form, category: e.target.value})} className="w-full border border-gray-300 px-3 py-2 outline-none focus:border-[#125ab2] font-black text-gray-700 bg-white rounded-sm shadow-sm cursor-pointer text-xs">
                      {options.categories.map(c => <option key={c} value={c}>{c}</option>)}
                    </select>
                  </div>
                  <div>
                    ...
                    <input type="text" value={form.contact} onChange={e => setForm({...form, contact: e.target.value})} placeholder="Company Name or Individual..." className="w-full border border-gray-300 px-3 py-2 outline-none focus:border-[#125ab2] font-bold text-gray-900 rounded-sm shadow-sm text-sm" required autoFocus/>
                  </div>
                </div>

                {/* ROW 2: Subject */}
                <div>
                  <label className="block text-[9px] font-bold text-gray-500 uppercase tracking-wider mb-1.5">Call Subject / Agenda <span className="text-red-500">*</span></label>
                  <input type="text" value={form.subject} onChange={e => setForm({...form, subject: e.target.value})} placeholder="Purpose of this communication..." className="w-full border border-gray-300 px-3 py-2 outline-none focus:border-[#125ab2] font-semibold text-gray-800 rounded-sm shadow-sm text-sm" required/>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div>
                    ...
                    <input type="date" value={form.date} onChange={e => setForm({...form, date: e.target.value})} className="w-full border border-gray-300 px-3 py-2 outline-none focus:border-[#125ab2] font-mono text-xs font-bold text-gray-800 rounded-sm shadow-sm cursor-pointer" required/>
                  </div>
                  <div>
                    ...
                    <input type="time" value={form.time} onChange={e => setForm({...form, time: e.target.value})} className="w-full border border-gray-300 px-3 py-2 outline-none focus:border-[#125ab2] font-mono text-xs font-bold text-gray-800 rounded-sm shadow-sm cursor-pointer" required/>
                  </div>
                  <div>
                    ...
                    <select value={form.status} onChange={e => setForm({...form, status: e.target.value})} className="w-full border border-gray-300 px-3 py-2 outline-none focus:border-[#125ab2] font-black text-gray-700 bg-white rounded-sm shadow-sm cursor-pointer text-xs">
                      {options.statuses.map(s => <option key={s} value={s}>{s}</option>)}
                    </select>
                  </div>
                </div>
                ...
                {/* ROW 4: Audit Notes */}
                <div className="bg-gray-50 border border-gray-200 p-4 rounded-sm shadow-inner mt-2">
                  <label className="block text-[9px] font-black text-gray-600 uppercase tracking-wider mb-2">Meeting Outcome / Audit Notes</label>
                  <textarea rows="3" placeholder="Record agreed terms, delays, or follow-up actions here..." value={form.notes} onChange={e => setForm({...form, notes: e.target.value})} className="w-full border border-gray-300 px-3 py-2 outline-none focus:border-[#125ab2] rounded-sm resize-none text-xs font-medium bg-white shadow-sm"></textarea>
                </div>
              ...
              <div className="bg-gray-50 border-t border-gray-200 px-6 py-4 flex justify-end gap-3 shrink-0 z-10">
                ...
                <button type="submit" className="w-full sm:w-auto px-6 py-2.5 bg-[#125ab2] hover:bg-[#0e4487] text-white text-[10px] font-bold uppercase tracking-wider shadow-sm transition-colors rounded-sm flex items-center justify-center gap-2">
                  {editingId ? 'Update Communication Log' : 'Commit to Ledger'}
                </button>
              </div>
            </form>
          </div>
