...
import Login from './Login'; // <-- INJEKSI: Halaman Login
import Sidebar from './Sidebar'; // <-- INJEKSI: Navigasi Kiri (RBAC)
import Header from './Header';
// import NavBar from './NavBar'; // <-- NavBar dinonaktifkan karena sudah diganti Sidebar
...
function AppContent() {
  // ─── 🚀 INJEKSI FASE 10: Menarik status 'user' dari Context ───
  const { halaman, setHalaman, user } = useContext(AppContext);

  // =========================================================================
  // 🛡️ THE GATEKEEPER: Jika tidak ada sesi user aktif, HANYA render Login
  // =========================================================================
  if (!user) {
    return <Login />;
  }

  // Zentryx Master Route Config (FITUR LAMA ANDA TETAP UTUH 100%)
  const pages = {
    ...
    dockSchedule: { component: DockScheduling, title: 'Dock Scheduling', breadcrumb: ['Schedule', 'Dock Scheduling'] },
  ...
  useEffect(() => {
    // 1. Baca URL saat tab baru dibuka
    const initHash = window.location.hash.replace('#', '');
    if (initHash && initHash !== halaman && pages[initHash]) {
      setHalaman(initHash);
    }

    // 2. Dengarkan navigasi Back/Forward dari browser
    const onHashChange = () => {
  ...
  useEffect(() => {
    // 3. Update URL browser secara senyap agar link bisa di-copy & dishare
    ...
    <div className="flex h-screen bg-[#f3f6f9] font-sans overflow-hidden text-sm">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <main className="flex-1 overflow-y-auto pb-10">
          {/* Sembunyikan Header Title untuk Dashboard & Scanner UI */}
