...
const Dashboard = () => {
  ...
  const lowStockAlerts = useMemo(() => {
    return (inventoryData || [])
      .filter(item => item.qty < 500 && item.qty > 0)
  ...
  // ─── SVG + HTML HYBRID CHART HELPERS ───
  // Menggunakan HTML untuk titik & teks agar tidak terdistorsi, dan SVG murni untuk garis
  const generateLineChart = (dataArray, color, fillOpacity) => {
    const max = Math.max(...dataArray);
    const min = Math.min(...dataArray);
    const range = max - min || 1;
    
    // Berikan ruang padding 20% di atas dan bawah agar titik tidak menempel tembok kontainer
    const padding = range * 0.2;
    const calcMax = max + padding;
    const calcMin = Math.max(0, min - padding);
    const calcRange = calcMax - calcMin || 1;
    
    // Helper persentase (0% sampai 100%)
    const getX = (i) => (i / (dataArray.length - 1)) * 100;
    const getY = (val) => 100 - (((val - calcMin) / calcRange) * 100);

    const points = dataArray.map((val, i) => `${getX(i)},${getY(val)}`).join(' ');

    return (
      <div className="absolute inset-0 w-full h-full pointer-events-none">
        {/* Layer 1: SVG Background (Garis dan Gradasi) */}
        ...
        {/* Layer 2: HTML Overlay (Titik Node dan Tooltip Hover) */}
        {dataArray.map((val, i) => (
          <div 
            key={i} 
            className="absolute bg-white rounded-full cursor-pointer pointer-events-auto transition-all duration-200 group z-10"
            style={{ 
              left: `${getX(i)}%`, 
              top: `${getY(val)}%`, 
              width: '10px', 
          ...
          >
            ...
            <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 bg-slate-800 text-white text-[10px] font-bold px-2 py-0.5 rounded shadow-lg opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none">
              {val}
            </div>
          </div>
        ))}
  ...
  const fillRateData = [88, 89, 90, 89, 91, 93, 92, 94, 94, 95, 94.2, 96]; 
  const otifData = [91, 92, 90, 94, 95, 96.5]; 
  const invLevelData = [4.2, 4.5, 4.3, 4.8, 5.1, 5.0, 4.9, 5.2, 5.5, 5.4, 5.8, 6.1]; 
  ...
  return (
    <main className="p-4 w-full max-w-[1400px] mx-auto relative z-0 animate-fade-in font-sans">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
        ...
        <div className="bg-white border border-gray-200 shadow-sm rounded-sm p-5 flex flex-col h-64 hover:shadow-md transition-shadow overflow-hidden">
          ...
          {/* min-h-0 adalah KUNCI pengaman Flexbox */}
          <div className="flex-1 relative min-h-0 w-full mt-2 mb-2">
            {generateLineChart(fillRateData, '#10b981', '0.15')}
          </div>
          <div className="flex justify-between text-[9px] font-bold text-gray-400 mt-4 border-t border-gray-100 pt-2 shrink-0">
        ...
        <div className="bg-white border border-gray-200 shadow-sm rounded-sm p-5 flex flex-col h-64 hover:shadow-md transition-shadow overflow-hidden">
          ...
          <div className="flex-1 relative min-h-0 w-full flex items-end justify-around gap-2 pt-2 border-b border-gray-100 pb-1">
            {backorderSkus.map((item, idx) => {
              const heightPct = (item.qty / maxBackorder) * 85; // Disisakan 15% di atas agar tooltip masuk
              return (
                <div key={idx} className="flex flex-col items-center w-full group h-full justify-end relative">
                  <span className="absolute bottom-full mb-1 text-[9px] font-black text-red-600 opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap">{item.qty}</span>
                  <div 
          ...
          <div className="flex justify-around text-[8px] font-bold text-gray-500 mt-2 shrink-0">
             {backorderSkus.map((item, idx) => (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white border border-red-200 shadow-sm rounded-sm flex flex-col overflow-hidden hover:shadow-md transition-shadow h-80">
          <div className="bg-red-50 border-b border-red-100 px-5 py-3.5 flex justify-between items-center shrink-0">
        ...
        {/* A2: Low Stock Alerts */}
        <div className="bg-white border border-amber-200 shadow-sm rounded-sm flex flex-col overflow-hidden hover:shadow-md transition-shadow h-80">
          <div className="bg-amber-50 border-b border-amber-100 px-5 py-3.5 flex justify-between items-center shrink-0">
        ...
        <div className="bg-white border border-gray-200 shadow-sm rounded-sm p-6 flex flex-col items-center justify-between hover:shadow-md transition-shadow relative overflow-hidden h-80">
          ...
          <div className="relative w-56 h-28 overflow-hidden mb-4 shrink-0">
          ...
          <div className="bg-gray-50 border border-gray-100 p-3 rounded-sm w-full text-center mt-auto shrink-0">
